...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> _processPayment() async {
    ...
    final Map orderConfirmation = {
      ...
      'itemCount': widget.cart.getTotalItems(),
  ...
  int _calculateItemPrice(Sandwich sandwich, int quantity) {
    PricingRepository repo = PricingRepository();
    return repo.calculateTotalPrice(
      quantity: quantity,
  ...
  Widget build(BuildContext context) {
    ...
    for (MapEntry<Sandwich, int> entry in widget.cart.items.entries) {
      ...
      final double itemPrice =
          _calculateItemPrice(sandwich, quantity) as double;
